...
bool CreateProfilePopup::setup(GJGameLevel *level)
{
  ...
  this->m_button1 = ButtonSprite::create("Create Profile", 0, 0, "goldFont.fnt", "GJ_button_01.png", 0.0f, .8f);
  ...
  lists->setContentWidth(m_mainLayer->getContentWidth() - 20.f);
  lists->setAnchorPoint({0.f, 1.f});
  lists->setPosition(10.f, m_mainLayer->getContentHeight() - 40.f);
  lists->setLayout(
      ColumnLayout::create()
          ->setGap(8.f)
          ->setAutoScale(false)
...
{
  std::string levelName = m_level->m_levelName;
  matjson::Value profile1 = generateProfile(fmt::format("2.1 {}", levelName), m_2_1_percentages);
  matjson::Value profile2 = generateProfile(fmt::format("2.2 {}", levelName), m_2_2_percentages);
